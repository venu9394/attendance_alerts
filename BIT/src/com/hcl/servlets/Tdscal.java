...
public class Tdscal extends HttpServlet
{
  /**
	 * 
	 */
...
public Tdscal() {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
  {
    ...
    session.setAttribute("message", "<p align='center'><a href='Tax1819.html'> Record Submition Failed Please Contact Admin/HR </a></p>");
    ...
    try
    {
      ...
      if ((emplinsert != null) && (emplinsert.equalsIgnoreCase("Save")))
      {
        ...
        saving.append(" (");
        saving.append("FY, EMPLOYEEID, NAME, DEPT, DESIGNATION, ADDRESS, CONTACT, HQ, AGE,PANNUMBER, STATUS, LUPDATE");
        saving.append(" )");
  ...
  /*    
      if(Integer.parseInt(Kidcount)==1||Integer.parseInt(Kidcount)==2)
      {
          ...
          try
          {
            ...
            if ((Kname != null) && (Kname.length() != 0)) {
              ps.executeUpdate();
              System.out.println("KID1"+ps.toString());
            }
          }
          catch (Exception p) {
            p.printStackTrace();
          ...
          if(Integer.parseInt(Kidcount)==2)
          {
          ...
          try {
            ...
            if ((Kname != null) && (Kname.length() != 0)) {
              ps.executeUpdate();
              System.out.println("KID2"+ps.toString());
            }
          }
          catch (Exception q) {
            q.printStackTrace();
      ...
      */
      ...
      try {
        ...
        ps.setString(5, Kclass1);
        ps.setString(6, Kschool1);
        if ((Kname1 != null) && (Kname1.length() != 0)) {
          ps.executeUpdate();
      ...
      System.out.println("RentHouse::"+RentHouse);
      ...
      if (ParamValues1 != null) {
        ...
        if (dependentFlag) {
          ...
          $80D_Depended = Double.valueOf(Math.min($80D_Depended.doubleValue(), 50000.0D));
        }
        if (selfflag) {
          $80D_Self = Double.valueOf(Math.min($80D_Self.doubleValue(), 25000.0D));
        } else {
          $80D_Self = Double.valueOf(Math.min($80D_Self.doubleValue(), 50000.0D));
        }
      }
  ...
  /*    tbl11.append(" (SELECT CASE WHEN MONTH(CURDATE())<=3 THEN CONCAT(YEAR(CURDATE())-1, '-',YEAR(CURDATE()))");
      ...
      */
      ...
      StringBuffer DisplayPage=new StringBuffer();
      if(i>0){
    	  try{
        		con.commit();
        	}catch(Exception err){
    	  ...
    	  System.out.println(id+"<------------>"+financialyear);
         DisplayPage.append("<form action='TDSDownloadReport' method='post'>");
         
         DisplayPage.append("<input type='text' value='"+id+"' name='empid' style='display:none' />");
         DisplayPage.append("<input type='text' value="+financialyear+"  name='FYEAR' style='display:none' />");
         DisplayPage.append("<div align='center' width='50%'> <br/><p style='color:green'> Record Submit Successfully </p><br/>");
         DisplayPage.append(" <P> <span style='color:red'>Please Download your Declaration form and submit to concern department </span> &nbsp;&nbsp;&nbsp; <input type='submit' value='Download'> <P> ");
         DisplayPage.append(" </div></form>");
         
        
        
       //  out.print(DisplayPage.toString());
         session.setAttribute("message", DisplayPage.toString());
      }else{
      	...
      	DisplayPage.append("<p align='center'><a href='Tax1819.html'> Record Submition Failed Please Contact Admin/HR </a></p>");
    ...
    //  out.print("<p align='center'><a href='TDSDownloadReport?empid="+id+"&FYEAR="+financialyear+"' > Record Submit Successfully </a> </p>");
    }
    catch (Exception e) {
    ...
    try{
    response.sendRedirect("SuccessFaild.jsp");
    }catch(Exception er){
    	er.printStackTrace();
    }
